...
local opened={};local menu={open=function(game,title,rows,opts)opened[#opened+1]={game=game,title=title,rows=rows,opts=opts};return rows end}
...
local callbacks={};local hooks={};local pickerCalls={};local invalidated=0;local preview=function()return true end
local songOpened=0
...
local api=Integration.attach(mod,{Version={get=function()return'firered'end,generation=function()return 3 end},Runtime=bus,
 ...
 drawAppearancePreview=preview,openSong=function()songOpened=songOpened+1 end,describeSong=function()return'PALLET TOWN'end})
...
ck(opened[#opened].opts and opened[#opened].opts.preview==preview,'Bike Appearance did not wire animated preview')
...
ck(audio[8].value()=='PALLET TOWN','Bike Song row did not use friendly selected-song label');audio[8].activate();ck(songOpened==1,'Bike Song row did not open song browser')
