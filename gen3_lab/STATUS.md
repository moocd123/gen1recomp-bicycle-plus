...
- `1eba54ff4ba54b66c2ea5369c3164584ddd18de5` added the native FireRed colour-picker foundation, remembered custom colours behind Original, paint-only reset and its regression tests. GitHub Actions run `35490539153` completed successfully against the pinned v0.2.66 engine.
- `b46d93526281f965f2b7ce934b9449e94f9c51e0` added an isolated mouse/touch press bridge for the picker because Game3 v0.2.66 does not publish the Gen1/2 `input.pointer` hook. It translates the current FireRed display fit back to native 240x160 coordinates, intercepts only an active picker inside the game viewport, and restores the original Game3 methods on disposal. GitHub Actions run `35490630228` completed successfully, including unchanged Gen1/2 regression suites and all FireRed component tests.
- `b5a2eb1f5e58853a1cb90935ceb4bc31e7493f62` completed continuous mouse/touch drag capture for the modal picker. Captured moves and releases remain isolated from native touch controls even when the pointer leaves the game viewport, synthetic touch-generated mouse events stay on the native path, and capture ends cleanly on release. GitHub Actions run `35492727489` completed successfully against the pinned engine, including the unchanged Gen1/2 regression suites and all FireRed component checks.

These tests are code/integration checks with generated fixtures. They do not claim ROM-dependent in-motion mask review, physical touch-device testing, or listening tests.
...
- integration.lua + beta_main.lua: FireRed generation gate and lifecycle wiring attach native auto-mount, add an AUTOBIKE+ row to the native Gen 3 OPTION screen, expose Gen 3-styled top/audio/appearance pages, wire the colour picker, provide paint-only reset, and restore direct engine wrappers on disposal. beta_manifest.json uses the distinct autobike_plus_firered_beta id and has no GitHub updater field.
...
Repository tests cover model rules, native Bag/ItemUse, native UI Stack, isolated settings, lifecycle/menu integration, native colour-picker state/input, FireRed display-coordinate pointer press/move/release capture and player-only/private-texture paint interception using generated RGBA bytes. Refer to Actions for actual pass/fail outcomes. A synthetic renderer test proves selection and source isolation, not the anatomical correctness of every real-art mask; that still requires visual review in motion.

## Still required before a playable beta

Add an in-menu animated bicycle preview; implement independent M4A sequencing/audio mixing; current/other-game/imported-file song selection and profiles; platform chooser/decoder integration; beta staging/ZIP validation; full-gameplay and physical-device tests. Reusing the singleton Gen3 audio worker would steal native music, so the Gen1/2 audio implementation cannot simply be enabled for FireRed.
