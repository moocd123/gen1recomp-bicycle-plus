-- Bridge Gen3 mouse/touch and raw-key input into AUTOBIKE+ modal pages without
-- modifying Game3. Captured pointers keep their modal owner through drags.
local Bridge={}
function Bridge.attach(mod,picker,services)
 ...
 local function stackPage()
  local top=Stack and Stack.top and Stack.top();local page=top and top.mod
  if page and page._autobikeGen3Pointer then return page end
 end
 local function currentPage()
  if disposed then return nil end
  local page=stackPage()
  if page and page.pointer then return page end
  if picker and picker.active then page=picker.active()end
  return page and page.pointer and page or nil
 end
 local function currentRawPage()
  if disposed then return nil end
  local page=stackPage()
  if page and page.rawKey then return page end
  if picker and picker.active then page=picker.active()end
  return page and page.rawKey and page or nil
 end
 local function dispatch(phase,id,x,y,needsCapture)
  ...
  else page=currentPage();if not page then return false end end
