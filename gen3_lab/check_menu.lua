...
local G={setColor=noop,rectangle=noop,push=noop,pop=noop,transformPoint=function(x,y)return x,y end,
 intersectScissor=function(x,y,w,h)assert(x>=0 and x+w<=240 and y>=0 and y+h<=160)end}
local W={template=function(...)return{...}end,userFrame=function(t,f)assert(f==4)end,
 printPx=function(s,x,y)draws[#draws+1]={s=s,x=x,y=y}end}
local m=Menu.new({Stack=Stack,Window=W,Chrome={fixedStdFrame=noop},
...
local rows={};local chosen=0
for i=1,10 do rows[i]={label='ROW '..i,value=function()return'ON'end,step=function(d)chosen=chosen+d end}end
local page=m.open({options={frameType=4}},'AUTOBIKE+ LAB',rows)
local input={pressed={}}
...
ck(Stack.top().mod==page,'native stack not used')
page.draw();tap('right');ck(chosen==1,'right did not adjust')
for i=1,60 do page.update(1/60);page.handleInput(input)end
ck(chosen==1,'holding caused repeat without new press')
