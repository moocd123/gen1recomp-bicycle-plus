-- Run from the repository root: texlua verification/v1.5.0/check_updater.lua /path/to/engine
...
check(current.version == '1.5.0', 'release version')
...
-- Release metadata is a local fixture of the intended v1.5.0 publication.
-- Real transport/install boundaries remain simulated in this state-machine test.
local repo, id = current.github, current.id
local release = {
  tag_name='v1.5.0', name='Bicycle Plus v1.5.0 test fixture', prerelease=false,
  assets={
    ...
    {name='bicycle_plus-1.5.0.zip',browser_download_url='https://github.com/'..repo..'/releases/download/v1.5.0/bicycle_plus-1.5.0.zip'}
  }
}
local parsed = assert(ModUpdate.parseRelease(release,id))
check(parsed.zip.name == 'bicycle_plus-1.5.0.zip', 'correct installable asset selected over unrelated ZIP')
check(parsed.version == '1.5.0', 'tag parsed')
check(ModUpdate.statusFor('1.4.2',{parsed}) == 'available', 'new release offered')
check(ModUpdate.statusFor('1.5.0',{parsed}) == 'current', 'no reinstall of current release')
check(ModUpdate.statusFor('1.5.1',{parsed}) == 'current', 'no downgrade')
...
local installedRaw=clone(raw);installedRaw.version='1.4.2'
local installed=Manifest.validate(installedRaw,'mods/bicycle_plus/manifest.json')
local imp=importer(installed)
...
check(installs[1].release.zip.name == 'bicycle_plus-1.5.0.zip','native install request selects exact ZIP')
...
local same=clone(raw);same.version='1.5.0'
...
local offline=importer(installed)
...
print('PASS: native Update All 1.4.2 -> 1.5.0 discovery/queue/completion; bootstrap, current/offline cases')
