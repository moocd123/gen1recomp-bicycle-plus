...
-- These are exposed forks/handlebar outlines. The apparent bar between
-- the wheels is actually animated shoe/leg artwork and must stay untouched.
local downFrame = {{6,12},{9,12},{6,13},{9,13},{6,14},{9,14}}
local upFrame = {{6,14},{9,14}}
local sideFrame = {{2,10},{3,10},{3,11}}
...
for kind, highlights in pairs(Parts.legacyMasks) do
  for frame=1,6 do
    ...
    if direction == 3 then
      ...
      outline = copy(sideFrame,leftShift)
    else
      tyres = copy(lowerTyre)
      centres = {}
      outline = copy(direction == 1 and downFrame or upFrame)
    end
    ...
    mask.lookup = {rims=index(rims),centres=index(centres),tyres=index(tyres),frame=index(outline)}
...
function Parts.classify(kind,frame,x,y,r,g,b,a,explicitStripe)
  ...
  if mask.rims[key] then
    ...
    -- All 26 verified Trainer Skins sheets use this peach wheel highlight;
  ...
  if not darkest then return nil end -- Hands/clothing/shoes are never paint.
